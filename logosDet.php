...
<script type="text/javascript" src="./js/jquery.js"></script>
  ...
  <div id="conteudo">
  	<div id="titulo-categoria">
        <?
            $SQL2 = "SELECT  nome AS titulo FROM categoria_logos WHERE id = $id;";
    	...
    	<div id="box-menuLateral">
            <ul>
            <?
            $SQL2 = "SELECT id AS catId, nome AS nomeCat FROM categoria_logos ORDER BY nome";
            $resultado2 = mysql_query($SQL2);
            while($linha2 = mysql_fetch_array($resultado2)){
                extract($linha2);
                ?><li><a href="logosDet.php?id=<?=$catId?>"><?=$nomeCat;?></a></li><?               
            }
            ?>            
            </ul>            
        ...
        <div id="box-contentDireita">
            ...
            <?
                $SQL2 = "SELECT * FROM logos WHERE categoria_logos_id = $id;";
                $resultado2 = mysql_query($SQL2);
                while($linha2 = mysql_fetch_array($resultado2)){
                    extract($linha2);
                    ?>
                    <li>
                        ...
                        <h5>Nome arquivo:</h5>
                        <h6><a href="./adm/logos/arquivos/<?=$id?>/<?=$imagem?>" title="Clique para baixar" target="_blank"><?=$imagem?></a></h6>
                    </li> 
                    <?               
                }
            ?>
